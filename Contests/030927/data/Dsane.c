...
if (cost[1] != 0) fprintf(stderr,"case %d energy0 %d\n",which,cost[0]);
if (cost[n] != 0) fprintf(stderr,"case %d energy0 %d\n",which,cost[n-1]);
      assert (cost[1] == 0 && cost[n] == 0);
      best[1] = -100;
      for (z=0;;z++) {
         progress = 0;
