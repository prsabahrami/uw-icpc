...
struct maxthing {
 ...
 int get(int a, int b) {
  int ans = -77777777;
  while (a%16&&a<b) ans >?= val[a++];
  while (b%16&&a<b) ans >?= val[--b];
  a/=16; b/=16;
  while (a%16&&a<b) ans >?= buc[a++];
  while (b%16&&a<b) ans >?= buc[--b];
  a/=16; b/=16;
  while (a%16&&a<b) ans >?= buc2[a++];
  while (b%16&&a<b) ans >?= buc2[--b];
  a/=16; b/=16;
  while (a<b) ans >?= buc3[a++];
  return ans;
 }
